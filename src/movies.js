...
function getAllDirectors(paraMovies) {
  varmoviesDirector = [];
  const varDirectors = paraMovies.filter((varMovie) => {
    if (varMovie.director.length != 0){
      varmoviesDirector.push(varMovie.director);
      return true;
    }
    else {
      return false;
  ...
  return varmoviesDirector;
}

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
function howManyMovies(paraMovies) {
  //varstevendramaMovies =[];
  const varstevendramaMovie = paraMovies.filter(varMovie => {
      if (varMovie.director === 'Steven Spielberg' && varMovie.genre.includes('Drama')){
        return true;
      }
      else {
        return false;
  ...
  return varstevendramaMovie.length;
}

// Iteration 3: All scores average - Get the average of all scores with 2 decimals
function scoresAverage(paraMovies) {
  let varaverageScores =0;
  if (paraMovies.length === 0) {
    return 0;
  };
  const scoresmovieAverage = paraMovies.reduce ((acc,val) => {
    if (val.score == null){
      return acc;
    }
    else { return acc + val.score};
    },0)/paraMovies.length;
  varaverageScores = parseFloat(scoresmovieAverage.toFixed(2));
  return varaverageScores;
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(paraMovies) {
  let varaverageScores = 0;
  const vardramaMovie = paraMovies.filter(varMovie => {
    if (varMovie.genre.includes('Drama')){
      return true;
    }
    else {
      return false;
  ...
  if (vardramaMovie.length === 0) {
    return 0;
  }
  const dramascoresAverage = vardramaMovie.reduce((acc,val) =>{
      return acc + val.score;
  },0)/vardramaMovie.length;
  varaverageScores = parseFloat(dramascoresAverage.toFixed(2));
  return varaverageScores;
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(paraMovies) {
...
function orderAlphabetically(paraMovies) {
  varMovies = [...paraMovies];
  varMovies.sort((a,b) => {
    return a.title.toLowerCase().localeCompare(b.title.toLowerCase());
  });
  let varCount = 0;
  let varfirsttwentyTitle = [];
  const varfirstTwenty = varMovies.filter((x) => {
      if (varCount < 20) {
        varCount++;
        varfirsttwentyTitle.push(x.title);
        return true;
      }
      else {
        return false;
...
function turnHoursToMinutes(paraMovies) {
  ...
  varmoviesDuration = varMovies.filter((varMovie) => {
      varhrIndex = varMovie.duration.indexOf('h');
     // console.log(varhrIndex);
      if (varhrIndex >= 0){
        varMovieHours = varMovie.duration[varhrIndex - 1];
        //console.log(varMovieHours);
        varparsedmovieHours = parseInt(varMovieHours,10);
        //console.log(varparsedmovieHours);
      
      varminendIndex = varMovie.duration.indexOf('min');
      if (varminendIndex >= 0){
        varminstartIndex = varMovie.duration.indexOf(' ');
        varmovieMinutes = varMovie.duration.substring(varminstartIndex, varminendIndex);
        varparsedmovieMinutes = parseInt(varmovieMinutes,10);
       // console.log(varparsedmovieMinutes);
        vartotalMinutes = varparsedmovieMinutes + varparsedmovieHours * 60;
       // console.log(vartotalMinutes); 
        varMovie.duration = vartotalMinutes;
        return true;
      }
      else {
        vartotalMinutes = varparsedmovieHours * 60;
        varMovie.duration = vartotalMinutes;
        return true;
      }
  }
